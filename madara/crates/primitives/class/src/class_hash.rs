...
use crate::{
    ...
    mainnet_legacy_class_hashes::correct_legacy_class_hash_if_needed,
...
impl LegacyContractClass {
    pub fn class_hash(&self) -> Result<Felt, ComputeClassHashError> {
        ...
        .class_hash()?;
        let corrected_class_hash = correct_legacy_class_hash_if_needed(computed_class_hash);
...
impl CompressedLegacyContractClass {
    pub fn compute_class_hash(&self) -> Result<Felt, ComputeClassHashError> {
        ...
        let corrected_class_hash = correct_legacy_class_hash_if_needed(class_hash);
        Ok(corrected_class_hash)
    }
}
